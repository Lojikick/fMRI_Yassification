...
import torch
print("Is cuda properly installed", torch.cuda.is_available())  # Should print True if CUDA is properly installed
...
print("Testing if numpy is installed")
# Test NumPy
print("NumPy version:", np.__version__)

# Test PyTorch with NumPy interaction
arr = np.array([1, 2, 3], dtype=np.float32)
tensor = torch.from_numpy(arr)
print("Successfully converted NumPy array to PyTorch tensor:", tensor)
...
class batch_generator_external_images(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # Convert to numpy array and explicitly specify the dtype
        img = np.array(img, dtype=np.float32)  # Convert to float32 first
        # Convert to tensor
        img = torch.from_numpy(img)  # Use from_numpy instead of tensor()
        return img
        #img = img/255
        #img = img*2 - 1
...
print ("Latent variables successfully extracted!")
